...
:bintraypage: https://bintray.com/jmini/maven/asciidoctorj-git-link
...
This extension is built and tested with version `{asciidoctorj-version}` of asciidoctorj.
See the corresponding line in the `dependencies` section of the gradle build file:

----
include::{rootdir}/build.gradle[tags=adocj-dep,indent=0]
----
...
Run gradle at the root of the repository:

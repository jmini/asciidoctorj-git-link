...
There is one exception to this rule: if the file path (`docfile`) is located into 2 folders called `target/checkout`.
In that case those folders are also considered as a possible git clone location.
This might be useful during maven releases.

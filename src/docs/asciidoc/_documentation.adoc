...
In oder for the plugin to work, the file that is processed (value taken from the `{docfile}` attribute) needs to be located in one folder having the value defined by `<git-repository>` as name.
